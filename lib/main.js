...
function processTabs() {
  ...
  tabTotal = 0;
  let f = gMgr.enumerateReporters();
  while(f.hasMoreElements()) {
    var mr = f.getNext().QueryInterface(Ci.nsIMemoryReporter);
...
function flushTransmission(transmission) {
  transmission = {"version": brandShortName};
}
...
function onStatsPageOpened(tab) {
  ...
  var worker = tab.attach({
    ...
    onMessage: function () {
      ...
      flushTransmission(transmission);
      //transmission = {};
    }
  });
