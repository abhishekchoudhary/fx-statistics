...
function processReporters() {
  ...
  singleRepProperties = ["path", "amount"];
  while (e.hasMoreElements()) {
    let rOrig = e.getNext().QueryInterface(Ci.nsIMemoryReporter);
    /*for (key in rOrig) {
      if (singleRepProperties.indexOf(key) > -1)
        continue;
      else
        delete rOrig[key];
    }*/
    ...
    if (!rOrig["path"].contains("explicit/"))
      dataArray.push({"name": rOrig["path"], "size": rOrig["amount"]});
  }
  ...
  for(var t in tabCollector) {
    let a = fixTab(t);
    if (a)
      tabArray.push({"name":a, "size":tabCollector[t]});
    else
...
function flarify(obj) {
  properties = ["name", "children", "size"];
  
  obj["name"] = obj["_unsafeName"];
  delete obj["_unsafeName"];
  
  if (!!obj["_kids"]) { 
    obj["children"] = obj["_kids"];
    delete obj["_kids"];

    for (kid in obj["children"]) {
      flarify(obj["children"][kid]);
    }
  }
  else {
    obj["size"] = obj["_amount"];
    delete obj["_amount"]
  }
...
//for (var branch in tree["Main Process"]["_trees"]["explicit"])
  //console.log(branch);
